...
C = LIIS()
X = LIIS()
Clist = [[] for _ in range(N+1)]    #色Cが何番目にあったか
Xlist = [[] for _ in range(N+1)]    #番号Xが何番目にあったか
...
OD = OrderBIT(list(range(N+1)))

cnt = 0
ans = 0
for i in range(N):
    for x in Xlist[i]:
        #自身より小さい順番でまだ位置が確定していない色の数
        t = M[C[x]].count_lower(x-1)
        ans += x - OD.count_lower(x-1) - t
        M[C[x]].delete_val(x)
        OD.insert_val(x)
        cnt += 1
print(ans)